...
def get_registree_emails(filter_attributes=[]):
    ...
    emails = list(
        set(
            [
                r.email
                for r in dbh.get_all_registrees()
                if all([bool(getattr(r, a)) == v for (a, v) in fas])
            ]
        )
    )
    emails.sort()
...
def send_email_to_registrees(text_file, filter_attributes=[]):
    try:
        ...
        raise
        raise Error(
            f'Specified text file "{text_file}" does not exist or cannot be opened'
        )
    emails = get_registree_email_string(filter_attributes=filter_attributes)
    pyperclip.copy(emails)
...
if __name__ == "__main__":
    ...
    parser_registree_emails.add_argument(
        ...
        "--unpaid_only",
        action="store_true",
        help="Only provides email for people who have not paid in full",
    )
